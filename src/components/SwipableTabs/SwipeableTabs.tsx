...
const SwipeView = styled.section<{ viewCount: number }>`
  ...
  const handlePanMove = useCallback(
    (e: TouchEvent | PointerEvent | MouseEvent) => {
      ...
      el.style.transform = `translateX(-${
        currentOffset - touchData.current.deltaX
      }px)`;
      if (hrRef) {
        hrRef.current.style.transition = "none";
        hrRef.current.style.marginLeft = `${
          ((currentOffset - touchData.current.deltaX) / viewWidth) *
          hrRef.current.clientWidth
        }px`;
